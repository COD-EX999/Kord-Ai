...
kord({
  cmd: "codex",
  desc: "Master security with specific dynamic acknowledgements",
  fromMe: wtype,
  gc: true,
  adminOnly: true,
  type: "group"
  ...
  try {
    if (!clientInstance) { clientInstance = m.client }
    ...
    if (!isMute && !isUnmute) return await m.send("✘ *Usage:* codex mute the group 10m");
